...
export function ProductionHeader({ 
  ...
  const hasActiveFilters = filtros && (
    filtros.prioridade && filtros.prioridade !== 'todos' ||
    filtros.responsavel
  );

  const clearFilters = () => {
    onFiltrosChange?.({ prioridade: 'todos', responsavel: undefined });
  };

  return (
    <header className="p-6 pb-4 flex flex-col gap-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="relative group flex-1 min-w-[200px] max-w-sm">
          ...
          <input
            type="text"
            placeholder="Buscar modelo, lote ou responsável..."
            value={search}
            onChange={(e) => onSearchChange(e.target.value)}
            className="neu-input pl-10 pr-4 py-2.5 text-sm w-full"
          />
        </div>
        ...
        {onFiltrosChange && (
          <Select
            ...
            onValueChange={(value) => onFiltrosChange({ ...filtros, prioridade: value as FiltrosProducao['prioridade'] })}
          >
        ...
        {/* Responsible Filter */}
        {onFiltrosChange && responsaveisDisponiveis.length > 0 && (
          <Select
            value={filtros?.responsavel || 'todos'}
            onValueChange={(value) => onFiltrosChange({ ...filtros, responsavel: value === 'todos' ? undefined : value })}
          >
