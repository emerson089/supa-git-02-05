-- Migração para criar a tabela de comprovantes e seus status
-- Verificando se o tipo já existe antes de criar
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'comprovante_status') THEN
...
-- Trigger para updated_at automático (assumindo que public.update_updated_at_column() já existe)
...
-- Policies
DROP POLICY IF EXISTS "Admins e Gerentes podem gerenciar comprovantes" ON public.comprovantes;
...
CREATE INDEX IF NOT EXISTS idx_comprovantes_status ON public.comprovantes USING btree (status);
CREATE INDEX IF NOT EXISTS idx_comprovantes_created_at ON public.comprovantes USING btree (created_at);
